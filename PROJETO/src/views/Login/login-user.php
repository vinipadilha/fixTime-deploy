...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($stmt->num_rows > 0) {
    ...
    if (password_verify($senha_usuario, $hash_senha)) {
      ...
      header("Location: /src/views/main-page/Cliente/main.php");
      return;
    } else {
    ...
    <form method="POST" id="loginForm" class="space-y-3">
      ...
      <div>
        <p class="text-sm font-light text-gray-500">
          Ainda não tem conta? <a href="/src/views/Login/cadastro-user.php" class="font-medium hover:underline text-blue-500">Crie seu cadastro.</a>
        </p>
      </div>
    </form>
  </div>
